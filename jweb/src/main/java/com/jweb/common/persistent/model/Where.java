...
public class Where {
	...
	public static Where create(String name,Expression ex,Object value) {
		return new Where().and(name,ex,value);
	}
	...
	public static PrepareSql toSqlAndParams(Where where,PrepareSql prepareSql) {
		...
		if(where.value instanceof List) {
		...
		}else if(where.value!=null && where.value.getClass().isArray()){
			...
			if(where.value==null){
				if(where.ex.value().contains("!="))
					sql = StringUtil.toSlide(where.name) +" is not null ";
				else
					sql = StringUtil.toSlide(where.name) +" is null ";
			}else {

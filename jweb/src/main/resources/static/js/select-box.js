function SelectBox(settings){
	...
	self.render();
	if(self.defaultSetting.onChange){
		self.defaultSetting.onChange(self.defaultSetting.des);
	}
	return self;
};
SelectBox.prototype={
	...
	render:function(){
        ...
        if(this.defaultSetting.multiSelect){
        	$leftTh.find(".control-btn").text("全选").click(function(){
            	this.defaultSetting.des = [].concat(this.defaultSetting.src);
            	if(this.defaultSetting.onChange){
            		this.defaultSetting.onChange(this.defaultSetting.des);
            	}
            	this.renderData();
            }.bind(this));
	...
	renderData:function(){
        ...
        var search = this.defaultSetting.search;
        var cols = this.defaultSetting.cols;
        ...
        $leftBody.find(".control-btn").click(function(){
        	for(var i=0;i<self.defaultSetting.src.length;i++){
        		if(self.defaultSetting.src[i][self.defaultSetting.id]==$(this).attr("rowid")){
        			if(!self.defaultSetting.multiSelect){
        				self.defaultSetting.des=[];
        			}
        ...
        trs='';
        for(var i=0;i<des.length;i++){
        	if(desKey&&des[i][search].indexOf(desKey)==-1) continue;
        	trs+='<tr>';
        	for(var j=0;j<cols.length;j++){

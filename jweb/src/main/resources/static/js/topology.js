...
var Topology = function () {
  ...
  _createClass(Topology, [{
    ...
    value: function init(params) {
      ...
      this.xInDocument = container.offsetLeft;
      this.yInDocument = container.offsetTop;
      ...
      // 缩放比例
      this.scale = Math.min(this.viewWidth / this.contentWidth, this.viewHeight / this.contentHeight);
    ...
    value: function onMouseWheel(e) {
      if (e.delta > 0) {
        // 放大拓扑图
        this.scale = this.scale + 0.1 < 2 ? this.scale + 0.1 : 2;
        this.render();
      } else {
        // 缩小拓扑图
    ...
    value: function onMouseMove(e) {
      ...
      if (this.isDragging) {
        ...
        if (this.nodeInHover) {
          var _getRevisedPosition = this.getRevisedPosition(e),
              revisedX = _getRevisedPosition.x,
              revisedY = _getRevisedPosition.y;

          var _transformRPToTP = this.transformRPToTP(revisedX, revisedY),
    ...
    value: function getEventNode(e) {
      ...
      var _getRevisedPosition2 = this.getRevisedPosition(e),
          revisedX = _getRevisedPosition2.x,
          revisedY = _getRevisedPosition2.y;

      var _transformRPToTP2 = this.transformRPToTP(revisedX, revisedY),
    ...
    // 校正事件在视图层上的坐标，仅进行目标判断时需要校正

  }, {
    key: 'getRevisedPosition',
    value: function getRevisedPosition(e) {
      return {
        x: e.clientX - this.xInDocument,
        y: e.clientY - this.yInDocument
      };
    }
    // 转化视图层坐标到拓扑图层坐标
